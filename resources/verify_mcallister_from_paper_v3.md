# Full Pipeline: Verify McAllister W₀ Computation (v3)
...
There are **THREE distinct volumes** in play. Confusing them will give wrong V₀.

| Symbol | Value | Definition | Source |
|--------|-------|------------|--------|
| V_cytools | ≈ 4.10 | CYTools `compute_cy_volume(t)` at unit moduli | CYTools output |
| V[0] | 4711.83 | String-frame Kähler potential volume | eq. 4.3, cy_vol.dat |
| V_Einstein | ≈ 5.4×10⁶ | Einstein-frame volume V_E = V[0] × g_s^{-3/2} | eq. 2.14 |

**Relationships**:
```
V_Einstein = V[0] × g_s^{-3/2}
           = 4711.83 × (0.00911134)^{-3/2}
           = 4711.83 × 1148.7
           ≈ 5.41 × 10⁶
```

**IMPORTANT**: The file `cy_vol.dat` contains **V[0] = 4711.83**, which is the string-frame
volume that enters the Kähler potential (eq. 4.3) and V₀ formula (eq. 6.24).
It is NOT the Einstein-frame volume.
...
def get_intersection_tensor(cy):
    ...
    if isinstance(int_nums, dict):
        # Dict format: {(i,j,k): value}
        for (i, j, k), val in int_nums.items():
            if i < h11 and j < h11 and k < h11:
                # Intersection numbers are fully symmetric
                kappa[i, j, k] = val
                kappa[i, k, j] = val
                kappa[j, i, k] = val
                kappa[j, k, i] = val
...
def compute_W0_analytic():
    ...
    CRITICAL: Use mpmath for arbitrary precision. W₀ ~ 10⁻⁹⁰ requires
    ~100 decimal places. Standard float64 cannot represent this.
    """
...
def compute_V0_AdS(W0, g_s, V_0, kappa, p):
    ...
    Args:
        ...
        V_0: String-frame volume V[0] from cy_vol.dat (4711.83)
    ...
    V_bracket_0 = mpf(str(V_0))  # V[0] from cy_vol.dat (string-frame)
    W0_mp = mpf(str(W0))
    g_s_mp = mpf(str(g_s))
    ...
    print(f"  V[0] = {float(V_bracket_0):.2f} (string-frame, from cy_vol.dat)")
    print(f"  e^{{K₀}} = {e_K0:.6f}")
...
def verify_results(computed_g_s, computed_W0, computed_V0):
    ...
    W0_ok = W0_log_error < 0.5  # Within half an order of magnitude
...
def main():
    print("="*70)
    print("McAllister W₀ Reproduction (v3)")
    ...
    V_0 = 4711.83  # V[0] from cy_vol.dat (string-frame volume)
    V0 = compute_V0_AdS(
        result['W0'],
        result['g_s'],
        V_0,
...
```
======================================================================
McAllister W₀ Reproduction (v3)
Polytope: 4-214-647 (dual with h¹¹=4, h²¹=214)
======================================================================

[Phase 1] Loading geometry and verifying basis alignment...
Note: Converted simplices from 1-indexed to 0-indexed
✓ Loaded CY: h¹¹=4, h²¹=214
det(N) = XXX.XXXXXX
...
V₀(AdS) computation (eq. 6.24):
  V[0] = 4711.83 (string-frame, from cy_vol.dat)
  e^{K₀} = 0.236100
  g_s^7 = 5.05e-15
  (4 V[0])² = 3.55e+08
  W₀² = 5.29e-180
...
### 2. Volume Confusion
Three different volumes exist. `cy_vol.dat = 4711.83` is V[0] (string-frame), NOT V_Einstein.
...
W₀ ~ 10⁻⁹⁰ requires mpmath with ~150 decimal places. float64 silently underflows to 0.
